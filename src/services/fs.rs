use mlua::{Lua, Result, UserData, UserDataMethods, Function};
use std::fs;
use gio::prelude::*;
...
pub fn register(lua: Rc<Lua>) -> Result<()> {
    ...
    fs_table.set("watch", lua.create_function({
        let lua_rc = lua.clone();
        move |_, (path, callback): (String, Function)| {
            let file = File::for_path(&path);
            let monitor = file.monitor(gio::FileMonitorFlags::WATCH_MOVES, None::<&gio::Cancellable>)
                .map_err(mlua::Error::external)?;

            let callback_key = lua_rc.create_registry_value(callback)
                .map_err(mlua::Error::external)?;
